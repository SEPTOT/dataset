...
if st.button("Predict Genre"):
    try:
        if song_title in label_encoders['Song_Title'].classes_:
            song_title_enc = int(label_encoders['Song_Title'].transform([song_title])[0])
        else:
            song_title_enc = 0

        if artist in label_encoders['Artist'].classes_:
            artist_enc = int(label_encoders['Artist'].transform([artist])[0])
        else:
            artist_enc = 0

        if platform in label_encoders['Platform'].classes_:
            platform_enc = int(label_encoders['Platform'].transform([platform])[0])
        else:
            platform_enc = 0
